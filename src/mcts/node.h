...
class EdgeAndNode {
 public:
  ...
  // Returns U = numerator * p / sqrt(N).
  // Passed numerator is expected to be equal to (cpuct * sqrt(log(N[parent])).
  float GetU(float numerator) const {
    const auto u = numerator * GetP();
    if (u == 0) return u;
    return u / std::sqrt(GetNStarted());
  }
